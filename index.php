  ...
  <header class="fixed top-0 left-0 right-0 z-50 glass-sky border-b border-[#D8E8F5] dark:border-slate-800 transition-all duration-300 shadow-sm">
    <div class="h-16 w-full max-w-[1520px] mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-2 lg:gap-3">
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <?php if ($user): ?>
          ...
          <div class="relative group">
            <button class="flex items-center gap-2 p-1 rounded-xl bg-white dark:bg-slate-800 border border-[#D5E6F5] dark:border-slate-700 shadow-sm">
              <div class="w-8 h-8 rounded-xl bg-brand-primary text-white font-bold text-xs flex items-center justify-center shadow-sm">
                <?= strtoupper(substr($user['full_name'], 0, 1)) ?>
              </div>
              <div class="hidden md:flex flex-col text-left pr-1">
                <span class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate max-w-[120px]"><?= htmlspecialchars($user['full_name']) ?></span>
        ...
        <?php else: ?>
          <!-- Sign In Button -->
          <a href="login.php" class="hidden sm:inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-white dark:bg-slate-800 border border-[#D5E6F5] dark:border-slate-700 text-slate-800 dark:text-slate-200 hover:bg-brand-tint font-headline text-xs font-bold shadow-sm transition-all whitespace-nowrap">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="px-3.5 py-1.5 rounded-xl bg-white dark:bg-slate-800 border border-[#CAE7F7] dark:border-slate-700 text-xs font-bold text-brand-deep dark:text-sky-300 shadow-sm">
              <?= htmlspecialchars($user ? ($user['full_name'] . ' (' . ($user['institution_name'] ?? 'VSIT') . ')') : 'Parnavi Janbhor (VSIT)') ?>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div class="lg:col-span-2 p-6 sm:p-8 rounded-2xl glass-card border-2 border-brand-pastel dark:border-sky-500/40 shadow-sky-md flex flex-col justify-between relative overflow-hidden">
            ...
            <div class="relative z-10">
              ...
              <div class="mb-6">
                <span class="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Matched Competencies for <?= htmlspecialchars($user['full_name'] ?? 'Parnavi Janbhor') ?></span>
          ...
          <div id="portal-view-student" class="portal-view block space-y-6">
            <div class="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[#E2EFF8] dark:border-slate-700">
              <div>
                ...
                <h3 class="font-headline font-bold text-xl text-slate-900 dark:text-white"><?= htmlspecialchars(($user['full_name'] ?? 'Parnavi Janbhor') . ' | ' . ucfirst($user['role'] ?? 'Student') . ' Innovator Lead (' . ($user['institution_name'] ?? 'VSIT') . ')') ?></h3>
              </div>
              <div class="flex items-center gap-3">
